...
\note{
  ...
  figures, we can infer that these correspond to the proimal locus. At
...
\note{
  ...
  one or multiple QTL, we use multivariate QTL analysis with a one- or
  two-QTL model. In the two-QTL model, each expression trait is
  affected by one or the other QTL.

  A key technical difficulty is that this two-QTL model requires
  ...
  their estimated QTL locations and then consider only cutpoints on
  this list: $p-1$ partitions rather than $2^p$.
...
\note{
  ...
  On the left: We do a two-dimensional scan for the pair of locations
  under the two-QTL model, for each possible split of the transcripts
  into two groups. The blue and red curves are profiles for the
  ...
  the expression traits, considered individually. The triangles are
  the estimated locations of the two QTL, under the two-QTL
  model. There's a block triangle at the estimated QTL location for
  the single-QTL model.

  On the right: the difference between the LOD score for the two-QTL
  model for a given cutpoint and that for the single-QTL model, for
  each cut point.
...
\note{
Liver chromosom 7: good evidence for two QTL, but it's splitting off
just the first two expression traits. We should omit those two and
look for evidence for multiple QTL among the rest. The high LOD
differences that split at 25 suggests there would still be evidence
for multiple QTL, among those traits.
}
...
couple of expression traits. Again, it seems we might focus on a
more focused interval.
}
...
\note{
  ...
  Jianan did the bulk of the method development and data analysis.
  ...
  The project was a collaboration with Eric Schadt, who paid for the
  microarrays and genotyping when he was at Rosetta.
